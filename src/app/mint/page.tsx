...
export default function MintPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Generate Property ID (Simple timestamp-based ID for demo)
      const propertyId = `PROP-${Date.now()}`;

      console.log("Minting Deed:", { propertyId, metadataURI, valuation: formData.valuation });

      // 3. Call Contract
      const { hash, success } = await tokenizeProperty(
        propertyId,
        metadataURI,
        formData.valuation // Use valuation as the initial "price" or value recorded
      );
